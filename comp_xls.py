...
data_importer['Кількість'] = data_importer['Кількість'].apply(lambda x: str(x).split()[0].replace('.', ','))
data_client['Кількість'] = data_client['Кількість'].apply(lambda x: x.split(' ')[0])
